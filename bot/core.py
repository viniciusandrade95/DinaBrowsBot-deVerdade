...
from .conversation.flow_router import FlowRouter
from .conversation.state import ConversationState
from .schedule.repository import build_repository


agenda_repo = build_repository()
flow_router = FlowRouter(agenda_repo)


def _conversation_state(session: SessionState) -> ConversationState:
    convo = session.context.get("conversation_state")
    if not convo:
        convo = ConversationState()
        session.context["conversation_state"] = convo
    return convo
...
def handle_message(
    ...
    convo_state = _conversation_state(session)
    reply = flow_router.route(convo_state, text)

    # If new scheduling flow didn't handle, fall back to previous logic
    if reply == "Sou a Dina, posso ajudar a marcar ou cancelar sobrancelhas.":
